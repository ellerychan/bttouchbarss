...
SRC_URLS = (
    ...
    (
        "https://news.google.com/rss/search?q=when:24h+allinurl:reuters.com&ceid=US:en",
        "Reuters",
    ),
    ...
    ("https://rss.csmonitor.com/feeds/csm", "CSMonitor"),
    ...
    ("https://www.protocol.com/feeds/feed.rss", "protocol"),  # They have other feeds:  https://www.protocol.com/where-to-find-us
)
...
class RssDb:
    ...
    def create_db(self):
        ...
        with con:
            ...
            self.cur.execute(
                "CREATE TABLE Source(id INT PRIMARY KEY, name TEXT, abbrev TEXT, url TEXT, timestamp DATE)"
            )
            self.cur.execute(
                "CREATE TABLE Article(id INT PRIMARY KEY, title TEXT, url TEXT, source_id INT, FOREIGN KEY (source_id) REFERENCES Source(id))"
            )
        ...
        try:
            ...
            with self.conn:
                self.cur.execute(
                    ...
                    (source_id, d.feed.title, source_initials, source_url, updated),
                )
    ...
    def set_current_index(self, index):
        ...
        with self.conn:
            self.cur.execute(
                "UPDATE State SET value=? WHERE name='current_index'", (str(index),)
            )
    ...
    def add_to_history(self, new_article):
        ...
        articles = [str(new_article)] + self.get_history()[
            0 : MAX_HISTORY - 1
        ]  # only save last MAX_HISTORY ids
        with self.conn:
            self.cur.execute(
                "UPDATE State SET value=? WHERE name='history'", (" ".join(articles),)
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-c", "--create", action="store_true", help="(re-)create the feed database"
    )
    parser.add_argument(
        "-r", "--random", action="store_true", help="choose random article"
    )
    parser.add_argument(
        "-u", "--url", action="store_true", help="print current article URL to stdout"
    )
    parser.add_argument(
        "-n", "--next", action="store_true", help="choose the next article"
    )
    parser.add_argument(
        "-p", "--prev", action="store_true", help="choose the previous article"
    )
    parser.add_argument(
        "-wp",
        "--webpage",
    ...
    if args.create or not os.path.exists(DB_PATH):
        ...
        db.add_sources()
        logging.info(
            "Database (re)created: {} articles from {} feeds".format(
                db.get_article_count(), db.get_source_count()
            )
        )
    ...
    if args.random:
        ...
        print(title)
        logging.debug(
            "Choosing random article ({})".format(db.get_current_article_id())
        )
        if not title or len(title.strip()) < 1:
            logging.warning(
                "Article has blank title: ({}) {}".format(
                    db.get_current_article_id(), db.get_article_url()
                )
            )
        ...
        print(url)
        logging.debug(
            "Retrieving current article ({})".format(db.get_current_article_id())
        )
        ...
        if db.in_history():
            ...
            db.set_hold()  # assert hold to overcome a spurious -r fetch
        logging.info(
            "Choosing next article in history ({})".format(db.get_current_article_id())
        )
        # title = db.get_article_title()
        # print(title)
    elif args.prev:
        ...
        db.set_hold()
        logging.info(
            "Choosing prev article in history ({})".format(db.get_current_article_id())
        )
        # title = db.get_article_title()
        # print(title)
    elif args.webpage:
